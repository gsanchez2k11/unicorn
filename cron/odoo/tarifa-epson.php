...
<?php
/**
 * Grabamos los precios en odoo desde la hoja de cálculo creada a tal efecto.
 * Es muy importante copiar y pegar de manera manual la lista completa a otra hoja para que pueda ser procesada
 * @var [type]
 */
...
//$act_mpg = conectarOdoo::actualizar('default_code',$mpn,'barcode',$ean,'product.product'); //Lo deshabilito porque da error


if (preg_match('/EUR$/',$pvp) > 0) {
$pvp_tunning = moneda::cadenaAnumero(trim(substr($pvp,0,strlen($pvp)-3))); //Convertimos el precio a double
$act_precio = conectarOdoo::actualizar('default_code',$mpn,'lst_price',$pvp_tunning,'product.product'); //Actualizamos el precio basandonos en la referencia
