...
foreach ($articulos_coincidentes as $articulo) {
 /* $codes = array_column($articulo->custom_attributes,'attribute_code');
  ...
  //print_r($custom);
  //echo '</pre>';
  if (isset($custom['auto_actualizable']) && $custom['auto_actualizable'] == 1) { //Si es autoactualizable componemos el articulo para actualizar
...
$actualizar_precio = articulos::actualizarArticulo($sku,$info_actualizacion);    //Actualizamos el precio de venta
} 
