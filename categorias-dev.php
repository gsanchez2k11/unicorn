...
$('#arbol').bstreeview({ data: dameArbol() }); //Colocamos el arbol de categorias


/***********************************************************************************************************************************
 * ACCIONES
 ***********************************************************************************************************************************/
//CApturamos el click en la categoría de odoo
let catsOdoo = document.querySelectorAll('.cat-odoo'); //CAtegorias de odoo
catsOdoo.forEach(c => {
  c.addEventListener('click', function(e){ 
//Comprobamos si tenemos la tabla creada de antes y la limpiamos
if ($.fn.dataTable.isDataTable('#tablaArticulos') === true) {
  $('#tablaArticulos').DataTable().destroy();
      }
    let idClick = e.target.id.split('-')[2]; //Obtenemos la id de la categoria
    if (idClick !== undefined) { //Si tenemos la id pedimos los artículos
      let articulos = dameArticulos(idClick);
      console.log(articulos);

console.log($.fn.dataTable.isDataTable('#tablaArticulos'));
...
//Vamos a intentar generar un árbol de categorías
//let padre =  listaCategorias.filter(cat => cat.parent_id[0] === 1);
//console.log(padre);           
...
/*        for (articulo of articulos) { //Recorremos el listado de artículos
  ...
  <div class="row">
  <div class="col-md-3">
  <div id="arbol" style="font-size: 0.75em"></div>
  </div>
  <div class="col-md-9">
  <div class="w-auto card" >
          <div class="card-body">
          <div class="d-flex align-items-center m-3 visually-hidden" id="loader-buscando-articulos">
  <strong>Buscando artículos ...</strong>
          <div class="spinner-border text-info ms-auto" style="width: 3rem; height: 3rem;" role="status">
          </div>
</div>
          </div>
        <table id="tablaArticulos" class="display" width="100%"></table>
        <thead>
        <tr>
            <th>default_code</th>
        </tr>
