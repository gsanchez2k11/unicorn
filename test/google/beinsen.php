...
$cliente = conectar::ConectarBeinsen();    //Creamos la conexión
$service = new \Google_Service_Sheets($cliente); //Creamos el servicio
$rangos = conectar::TARIFABEINSENRANGOS;
$articulos = array();
foreach ($rangos as $rango) { //Recorremos el array con los rangos
    $resultado = $service->spreadsheets_values->get(conectar::TARIFABEINSENID, $rango);     //Pedimos a google los datos para ese rango
    $valores = array_filter($resultado->values);
    $articulos = array_merge($valores, $articulos); //Añadimos al array
    }

 $articulos = array_filter($articulos, function ($a) {
    return preg_match('/€$/',$a[3]); //Solo nos quedamos con aquellas filas en las que el precio de venta acaba en €
 });   
    echo "<pre>";
    print_r($articulos);
  echo "</pre>";
